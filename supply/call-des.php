	  	  	...
	  	  	$fslect = $pslect -> fetch(PDO::FETCH_ASSOC);
	  	  	 ...
	  	  	 <h5 class="card-title">Deliver Information</h5>
	  	  		<hr>
							    ...
							    <?php if ($fselevt['user_no'] == $_SESSION['user_no'] AND $fselevt['status'] == 'proposal'){ ?>
							    	<button id="<?php echo $fselevt['de_id'];?>" class="card-link btn btn-primary accept">Accept Delivery</button>
							    <?php }else if ($fselevt['user_no'] == $_SESSION['user_no'] AND $fselevt['status'] == 'inprogress'){
							    	$selefo = "SELECT count(*) as requestnum FROM request WHERE order_no='".$_GET['order_no']."'";
							    	$pselefo = $connect -> prepare($selefo);
							    	$pselefo -> execute();
							    	$fselefo = $pselefo -> fetch(PDO::FETCH_ASSOC);
							    	if ($fselefo['requestnum'] == 0) {
							    		?>
							    		<button id="<?php echo $fselevt['de_id'];?>" class="card-link btn btn-primary request">Send Request to customer</button>
							    		<?php
							    	}else if($fselefo['status'] == 'pending'){
                ?>
                <div class="mb-2"><strong>Wait for response</strong></div>
                <?php
							    	}else{
							    		?>
							    		<div class="mb-2"><strong>Delivery Well delivered</strong></div>
							    		<?php
							    	}
							    	?>
...
<?php
							    } else  if ($fselevt['user_no'] == $_SESSION['user_no'] AND $fselevt['status'] == 'request'){
							   ?>
							   <div class="mb-2"><strong><b><u>Wait for response</u></b></strong></div>
							   <?php
							     } 
							   }?>
						    </div><!--//app-card-body-->
