<?php 	include 'header.php'; ?>
						    ...
						    <div class="app-card-body">
							    <div class="table-responsive">
							        <table class="table app-table-hover mb-0 text-left">
										...
										<tbody>
											<?php $selef = "SELECT * FROM orders WHERE user_no ='".$_SESSION['user_no']."'";
											...
											<tr>
												...
												<td class="cell"><a class="btn-sm app-btn-secondary" href="new-order?order_no=<?php echo $fselef['order_no']; ?>">View</a></td>
											</tr>
										<?php } ?>
					    ...
					    <div class="app-card app-card-orders-table mb-5">
						    <div class="app-card-body">
								...
								<table class="table app-table-hover mb-0 text-left">
										...
										<tbody>
											<?php $selef = "SELECT * FROM orders WHERE status='success' AND user_no ='".$_SESSION['user_no']."'";
											$pselef = $connect -> prepare($selef);
											$pselef -> execute();
											while($fselef = $pselef -> fetch(PDO::FETCH_ASSOC)){ 
												?>
					    ...
					    <div class="app-card app-card-orders-table mb-5">
						    <div class="app-card-body">
								...
								<table class="table app-table-hover mb-0 text-left">
										...
										<tbody>
											<?php $selef = "SELECT * FROM orders WHERE status='pending' AND user_no ='".$_SESSION['user_no']."'";
											$pselef = $connect -> prepare($selef);
											$pselef -> execute();
											while($fselef = $pselef -> fetch(PDO::FETCH_ASSOC)){ 
												?>
					    ...
					    <div class="app-card app-card-orders-table mb-5">
						    <div class="app-card-body">
								...
								<table class="table app-table-hover mb-0 text-left">
										...
										<tbody>
											<?php $selef = "SELECT * FROM orders WHERE status='Cancelled' AND user_no ='".$_SESSION['user_no']."'";
											$pselef = $connect -> prepare($selef);
											$pselef -> execute();
											while($fselef = $pselef -> fetch(PDO::FETCH_ASSOC)){ 
												?>
 ...
 if (isset($_GET['Add_new'])){
  ...
  $select_orders = "SELECT *,count(*) as countin FROM orders WHERE user_no='$user_no' AND status='pending'";
  $prepare_orders = $connect -> prepare($select_orders);
  $prepare_orders -> execute();
  $fetch_orders = $prepare_orders -> fetch(PDO::FETCH_ASSOC);
  if ($fetch_orders['countin']!= 0) {
  	?>
    <script>
      window.alert('Please checkout this order first');
      window.location.href='new-order?order_no=<?php echo $fetch_orders['order_no'];?>&&/new_order/';
    </script>
    <?php
  }else{
  $inse = "INSERT INTO `orders`(`user_no`, `order_no`, `dates`, `total`, `method`, `delivery`, `status`) VALUES(:user_no,:s_no,:dates,:total,:pmethod,:delivery,:status)";
  $pinse = $connect -> prepare($inse);
  $pinse -> execute($datf);
  if ($pinse) {
    ?>
    <script>
      window.alert('sales created successful');
      window.location.href='new-order?order_no=<?php echo $s_no;?>&&/new_order/';
    </script>
    <?php
  }else{
    ?>
    <script>
      window.alert('Order refused to be created');
    </script>
    <?php
  }
}
